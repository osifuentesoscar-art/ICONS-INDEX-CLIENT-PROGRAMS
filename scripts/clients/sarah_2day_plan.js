 ...
 * Brace Life Studios
 *
 * FIRST BUILD 8/18/2026, REVISED SAME DAY (two new confirmed facts from
 * Xolokan — see "REVISION 8/18/2026" below). Prior to the first build the
 * only record of "Sarah" anywhere in this system was a two-line pre-repo
 * legacy outline in CLAUDE.md's old client-roster block (a virtual 2-day
 * "Athletic Strength" program built around a bench-press/cable-pull/
 ...
 * before any build" rule: her full existing record (this script + her full
 * CLIENTS.md entry) was read in full before making the revision below —
 * nothing on file was dropped or silently overwritten.
 *
 ...
 * feedback, not a formal assessment. Two demographic facts are now
 * confirmed directly by Xolokan (see revision note below) — sex, and full
 * home-gym equipment access. Age, weight, height, Styku scan, or
 * strength-testing numbers still do not exist anywhere in this system for
 * her. Per CLAUDE.md's Demographic Scope Rule, this program applies ONLY
 * the sex-neutral structural philosophy (Three-Zone build, RIR-based
 ...
 * Those thresholds require age and/or weight/Styku data in addition to
 * sex — confirming sex alone does NOT unlock any of them.
 * `includeNutritionBlock: false`; `client.weightKg`/`ageYears`/
 ...
 *
 * NICK'S STATED CONSTRAINTS (verbatim, the only real programming data on
 * file — nothing added beyond this, and NOT relaxed by the equipment
 * update below — see revision note):
 ...
 *     Goblet squat (light DB/KB up to 20 lbs) or bodyweight — no elevated-
 ...
 *
 * REVISION 8/18/2026 — two new confirmed facts from Xolokan, same day as
 * the first build:
 *   1. SEX: Sarah is female (`client.sex: 'female'` added). Documented
 *      explicitly (purple internal note below) that this alone does NOT
 *      unlock any Evidence-Based Science Layer numeric threshold — ALST,
 *      protein/creatine tiering, VFA, BMI flags, and LIFTMOR candidacy all
 *      also require age and/or weight/Styku data, none of which exists yet.
 *   2. EQUIPMENT: Sarah has a full standard home gym — NOT the minimal
 *      placeholder set assumed at first build. This is a deliberate
 *      trainer-programming picture, not an equipment gap: Nick's original
 *      constraints (no cable machine, 20 lb hard ceiling, avoid bench where
 *      possible, limited hip-hinge/squat ROM) are choices about what works
 *      for Sarah specifically, made MORE deliberate-looking now that we
 *      know equipment isn't the limiting factor — not less relevant. They
 *      are NOT relaxed here. Reworded the opening equipment baselineNote
 *      from an unconfirmed assumption ("if this is wrong, easy to correct")
 *      to a confirmed, deliberately-scoped equipment picture.
 *      Exercise-variety pass, evaluated against the constraints above:
 *        - ADDED: kettlebell offered as an equivalent-weight alternate
 *          implement to dumbbell for Goblet Squat, Suitcase Carry, and
 *          Farmer Carry (same pattern, same ≤20 lb ceiling — pure implement
 *          variety, no new movement pattern, no load change).
 *        - ADDED: elevated-surface language sharpened from generic
 *          "step/chair/couch" to a real step/box (DB Hip Thrust, Step-Ups),
 *          now that one is confirmed on hand rather than assumed.
 *        - ADDED: the pull-up bar is now named as the fixed overhead
 *          anchor for Banded Lat Pulldown, and the squat rack as the
 *          chest-height anchor for Standing Band Row — more stable/specific
 *          than an assumed door anchor, same band exercises, same loads.
 *        - CONSIDERED AND REJECTED: kettlebell swings as a metabolic-
 *          finisher option — a ballistic, hip-hinge-dominant movement that
 *          directly conflicts with constraints 4 and 5 (limited lower
 *          back/hamstring mobility, flat-back hinge struggle). Adding it
 *          now that a KB is available would be exactly the equipment-
 *          driven scope creep the main thread's instruction warned
 *          against.
 *        - CONSIDERED AND REJECTED: barbell, squat rack-loaded squat work,
 *          and bench work of any kind — all directly conflict with
 *          constraints 1, 3, and 6 (20 lb ceiling, avoid bench, limited
 *          squat depth). Their presence in the home gym changes nothing
 *          about why they're excluded.
 *      Net effect: NO change to sets, reps, tempo, rest, RIR targets, block
 *      structure, or exercise-pattern sequencing anywhere in this program —
 *      the Antagonist Rotation Rule analysis below is unaffected, since
 *      every change is an implement/anchor substitution within an already-
 *      approved movement pattern, not a new pattern. Load ceiling remains
 *      exactly 20 lbs everywhere, verified in this build's audit below.
 *
 * EQUIPMENT — CONFIRMED FULL HOME GYM (updated 8/18/2026, no longer an
 * assumption). Stated prominently in-document (first baselineNote, a
 * goldCallout): dumbbells and/or kettlebells up to 20 lbs (Nick's hard
 * ceiling, not an equipment limit), a resistance band, bodyweight, a
 * step/box, and the pull-up bar/squat rack as fixed band anchors. Two
 * pieces of her available equipment are deliberately NOT used anywhere in
 * this program, by Nick's direct programming choice, not availability: the
 * cable machine, and the bench (present in her gym, avoided per her stated
 * preference). No barbell-loaded work anywhere, consistent with the 20 lb
 * ceiling.
 *
...
const client = {
  name: 'Sarah',
  sex: 'female', // confirmed 8/18/2026 by Xolokan — does NOT alone unlock any
                 // numeric science-layer threshold; see the internal
                 // baselineNote below for why.
  programTitle: '2-Day Training Plan',
  subtitle: 'Fast-Paced Circuit Strength Foundation — Full Home Gym, Dumbbell/Kettlebell ≤20 lb, Band & Bodyweight',
  schedule: '2 Days/Week · Circuit Format',
  stats: [
    'Trainer: Nick',
    '2 Days/Week · Female',
    'Full Home Gym — Dumbbell/Kettlebell ≤20 lb (Trainer Ceiling), Band, Step/Box',
    'No age/weight/Styku data on file',
  ],
  ...
  // falsy-gated off these exact fields in icons_template.js. `sex` above
  // is purely a demographic record; icons_template.js has no logic keyed
  // off `client.sex`.
};
...
const days = [
  {
    ...
    descriptor: 'FAST-PACED CIRCUIT · HIGH REPETITION · DUMBBELL/KETTLEBELL ≤20 LB & BODYWEIGHT · 35–45 MIN',
    ...
    blocks: [
      ...
      // Standing Band Row (pull). No repeats. Compliant.
      {
        ...
        exercises: [
          { name: 'Suitcase Carry (Single-Side, DB or KB)', sets: '2-3 rounds', reps: '30-45s ea side', load: '≤20 lb', tempo: 'Controlled walk', rest: '15-20s then next', cue: 'Resist leaning toward the loaded side, tall posture, brace hard.' },
          ...
          { name: 'Standing Band Row (No Cable)', sets: '2-3 rounds', reps: '15-20', load: 'Light-Moderate Band', tempo: 'Controlled', rest: '45-60s (round rest) — then repeat carry', cue: 'Band anchored to the squat rack at chest height, elbows drive back, squeeze shoulder blades.' },
        ],
      },
    ],
  ...
  },
  {
    ...
    descriptor: 'FAST-PACED CIRCUIT · HIGH REPETITION · DUMBBELL/KETTLEBELL ≤20 LB, BAND & BODYWEIGHT · 35–45 MIN',
    ...
    blocks: [
      ...
      // same-pattern. Compliant.
      {
        ...
        exercises: [
          ...
          { name: 'Banded Lat Pulldown (No Cable)', sets: '3-4 rounds', reps: '15-20', load: 'Light-Moderate Band', tempo: 'Controlled', rest: '60-90s (round rest) — then repeat hip thrust', cue: 'Band anchored to the pull-up bar overhead, pull elbows down and back, squeeze the lats.', rirNote: '2-3 RIR' },
        ],
      },
      // Metabolic zone (less strict per CLAUDE.md, but checked anyway).
      ...
      // (lower/knee-dominant). No repeats. Compliant.
      {
        ...
        color: 'gold',
        introLabel: 'Format',
        intro: 'Shorter closing circuit — 2-3 rounds, same fast pace and minimal rest as Block B. Step-ups reuse the same step/box as the hip thrust above — no new equipment needed. This is the day’s primary conditioning stimulus at 2 days/week.',
        exercises: [
          { name: 'Farmer Carry (DB or KB, Both Hands)', sets: '2-3 rounds', reps: '30-45s', load: '≤20 lb/hand', tempo: 'Controlled walk', rest: '15-20s then next', cue: 'Shoulders packed, chest tall, carry evenly with both hands.' },
          ...
          { name: 'Step-Ups (Bodyweight, Step/Box)', sets: '2-3 rounds', reps: '10-12 ea leg', load: 'Bodyweight', tempo: 'Controlled', rest: '45-60s (round rest) — then repeat carry', cue: 'Full foot on the step/box, drive through the heel, step down with control.' },
        ],
      },
    ],
...
const summary = {
  ...
  rows: [
    ['Wk 1', '—', 'Day A & B', 'Establish comfortable working loads at 2-3 RIR', 'Confirm squat depth and hip-hinge tolerance before adding any load.'],
...
// Client View (added 8/18/2026, revised same day alongside the equipment/
// sex update): 2 of the 4 baselineNotes are marked audience: 'internal'
// (the science-layer-scope note and the data-provenance note) — both are
// build-process/documentation-methodology reasoning per CLAUDE.md's Client
// View spec, not client-facing coaching content. The 2 remaining notes
// (Equipment — Full Home Gym By Trainer Design, How This Program Was Built
// Around You) stay visible — genuinely useful, warmly-written client-facing
// content, reworded this revision to state the confirmed equipment picture
// rather than an unconfirmed assumption. No `insight`/`flag` fields carry
// build-rationale
