...
**Rollout status:** piloted first on one fully-instrumented client (Siobhan Hansen, 8/18–8/19/2026) before roster-wide application, per the same confirm-the-pilot-before-batch-producing discipline used for Client View and the Assessment Report. **Roster-wide rollout GREEN-LIT by Xolokan 8/19/2026** ("now we can green light the other clients docs to have this new method we just created") and **COMPLETED 8/21/2026** across four audited batches — 14 clients restructured, every one through build → independent audit → fix → deliver. Batch 1: Elizabeth Poyner, Kelly Mulroy (studio + travel), Johanna Castillo. Batch 2: Aimee Morris, Mary Burfete, Johnna Macarthur, Nicolette Scott (gym + at-home). Batch 3: Nancy Avitable, Rena Paul, August Olivia, Petra. Batch 4: Kayma Liburd, Jake Poyner, Vinz Feller, Moe Shahheidari. **Out of scope, each for a stated reason:** Sarah (trainer-directed circuit format stands, per the override above); the 10 intake-pending clients (build gate unmet — Styku scan AND strength battery both required); Daisy Auger Dominguez and Kerry Chandler (triaged 8/21 — neither has a build script, client directory, or document of any kind, only a legacy title, so there was nothing to restructure; their entries record what intake would unlock a real build). Clinically-led clients (Jake Poyner, Moe Shahheidari) got the clinician's structure first with this architecture fitted around it, per the override above. Each restructure also carried the standing "touch it, bring it current" pass: Kieser renames for cable references, DB-ceiling implement decisions, deload-week placement where criteria are met, 4-week cadence language, load-field Wk1→Wk4 convention, and the warm-up drift check. See CLIENTS.md for which clients have been restructured to this architecture and when.
