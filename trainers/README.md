# Trainers — Programs, Data & Education

**This tree holds everything trainer-side. Client material never lives here, and trainer material never lives under `clients/`.** That separation was made structural on 8/20/2026 (previously trainer programs sat in `trainer_education/` alongside the education modules, and one module was loose at the repo root).

```
trainers/
  <trainer_name>/   one folder per named trainer — their program .docx plus
                    any data we hold on them (baselines, scans, coach notes)
  education/        the trainer development curriculum: HTML knowledge
                    modules + physical "Train the Trainer" .docx programs
  README.md         this file
```

Build scripts are split the same way, so a trainer build can never write into a client folder:

```
scripts/icons_template.js      shared engine (both audiences)
scripts/clients/*.js           client builds   → clients/<client_name>/
scripts/trainers/*.js          trainer builds  → trainers/<trainer_name>/
                                                 trainers/education/
```

Current trainer roster with individual programs: **Becca, Brodie, Jah, Nick, Oscar** (Format 3 below). Jason Bethea (Trainer/Physical Therapist) and Niko Heers (Stretch Therapist) are studio staff who appear *in* client documents rather than carrying ICONS programs of their own — see CLAUDE.md's "STUDIO STAFF" section.
...
|---|---|---|
| `education/ICONS_Trainer_Learning_Module.html` | — | Base: 5 passive read-and-reflect modules, no gating. Start here. |
...
|---|---|---|
| `becca/Becca_3Day_Training_Plan.docx` | Intermediate | Deadlift/Split Stance/Row loads off her real 5RMs; Back Squat and Hex Bar Deadlift introduced as new conservative baselines (both were "Not Yet Assessed"); Full Pull-Up given a single unassisted test rep alongside her standing 3-grip assisted battery. |
| `brodie/Brodie_3Day_Training_Plan.docx` | Intermediate | Back Squat/Hex Bar Deadlift/Row loads off his real 5RMs; Conventional Deadlift introduced as a new baseline. Assisted Pull-Up deliberately NOT programmed — his 16-rep unassisted baseline already supersedes it. |
| `oscar/Oscar_3Day_Training_Plan.docx` | Intermediate | Back Squat/Hex Bar Deadlift/Row loads off his real 5RMs (heaviest row, highest pull-up count in the group); Conventional Deadlift and Farmer Carry both introduced as new baselines. Assisted Pull-Up deliberately skipped, same rationale as Brodie's. |
| `jah/Jah_3Day_Training_Plan.docx` | Intermediate/Advanced | Squat/Deadlift/Row/Split Squat loads off his real 5RMs; Goblet Squat and Hex Bar Deadlift introduced as new baselines per his own coach note's stated substitution intent (Hex Bar DL's Week 1 load explicitly derived from his Conventional Deadlift 1RM, not an independent number). |
| `nick/Nick_3Day_Training_Plan.docx` | Advanced/Elite | Same anchoring approach, but his coach note's "advanced periodization" instruction is applied structurally, not just narratively: Week 1 working loads computed at ~70/80/88% of 1RM (vs. the 60/70/80% used for the other 4), reps held to 3-5 throughout, rest at 120-180s on primary lifts (vs. 30-90s elsewhere), and one sub-maximal/maximal-intent velocity exercise per day (Trap Bar Jump, Med Ball Slam) using this system's existing Power Training framing. |
...
Build scripts: `scripts/trainers/becca_3day_plan.js`, `scripts/trainers/brodie_3day_plan.js`, `scripts/trainers/oscar_3day_plan.js`, `scripts/trainers/jah_3day_plan.js`, `scripts/trainers/nick_3day_plan.js`. Each writes to `trainers/<name>/`.

## Where a new trainer should start

1. `education/ICONS_Trainer_Learning_Module.html` — ungated foundation, 5 modules, ~100 min total.
