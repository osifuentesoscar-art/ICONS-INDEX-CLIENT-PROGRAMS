...
- **Antagonist rotation spot-check** (added 8/12/2026, at Xolokan's direction — see CLAUDE.md's "Compound Block Sequencing — Antagonist Rotation Rule"): for every Compound-zone block with 3+ exercises, read the exercise sequence and flag any three CONSECUTIVE exercises that load the same primary muscle group/movement pattern (e.g. three quad-dominant/hip-hinge-chain lifts in a row, three horizontal-push exercises in a row). Two in a row is fine — a stacked third is the defect. Isolated-zone/corrective blocks are exempt by design (they're often deliberately concentrated on one weak muscle group) — don't flag those. Also exempt: a deliberate grip/skill-progression sequence (the close/standard/wide-grip assisted pull-up battery, an incline-push-up→full-push-up→dip regression) — confirmed 8/12/2026 during the retroactive audit as intentional testing/progression content, not a stacked-compound defect. The retroactive audit of the 15 pre-existing documents (8/12/2026) is the reference example of how to run this check at scale.
